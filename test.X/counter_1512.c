...
void update_switches (void) {
    ...
    update_switch (p_btns[BTN_CAT_1], CAT_1, SWITCH_STREAK);
    update_switch (p_btns[BTN_CAT_2], CAT_2, SWITCH_STREAK);
    update_switch (p_btns[BTN_RST], RST, SWITCH_STREAK);
    ...
    if (get_switch_toggle(old_btns[BTN_CAT_1], v_btns[BTN_CAT_1])) {
        seg_disp_awake();
        change_category(0);
    }
    if (get_switch_toggle(old_btns[BTN_CAT_2], v_btns[BTN_CAT_2])){
        seg_disp_awake();
        change_category(1);
    }
    if (get_switch_status(v_btns[BTN_RST]) == 1 ){
        seg_disp_awake();
        extended_rst_streak++;
